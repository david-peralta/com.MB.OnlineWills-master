Feature: MW - 25 : As a customer, I want to enter the required details about my Relationship Status, so I can process my Will
  ...
  @Sprint1
  Scenario Outline: DivorcedCouples Scenario 2: Customer fills in additional details based on Relationship Status selected
    ...
    And user checks if additional questions displayed for divorced is correct when couple product is selected
  ...
  @Sprint1
  Scenario Outline: DivorcedSingles Scenario 2: Customer fills in additional details based on Relationship Status selected
    # Go to site and log in
    Given user opens browser
    When user logs into app with the "<email>" and "<password>" as the login credentials
    Then user is on "Landing" page
    ...
    When user selects "Divorced" as Relationship Status on about page
    #Check if additional questions is displayed
    And user checks if additional questions displayed for divorced is correct when single product is selected
